...
`TELEGRAM_BOT_TOKEN` variable in your `.env` file. The `initData` string from
Telegram is validated using the [`WebAppData` HMAC
procedure](https://core.telegram.org/bots/webapps#validating-data-received-via-the-web-app);
if the token does not match, the API responds with `{"error": "Invalid hash"}`.
`/api/telegram-auth/` accepts the `initData` value and returns a JWT token.
Users authenticated through Telegram are created automatically using their
Telegram ID.
